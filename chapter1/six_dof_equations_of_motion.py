...
from chapter1.aerodynamic_forces_moments import AerodynamicForcesMoments
...
from chapter1.propulsion_forces_moments import PropulsionForcesMoments
from DataDisplay.constants import EARTH_GRAVITY_VECTOR
from sim_math.kinematics import euler_kinematics, quaternion_kinematics
from sim_math.numeric_integration import rk4
from sim_math.rotation import euler2quat, rot_matrix_quat, rot_matrix_zyx
...
class EquationsOfMotion:
    def __init__(
        ...
        """
        Initialize the EquationsOfMotion class.
        ...
        self.params = params
        self.control_deflections = ControlDeflections(delta0)
        self.state = AircraftStates(x0, wind0, use_quat)
        self.aerodynamics = AerodynamicForcesMoments(params)
        self.propulsion = PropulsionForcesMoments(params)

    def set_state(self, x: np.ndarray) -> None:
        self.state.update(x)

    def set_control_deltas(self, delta: np.ndarray) -> None:
        self.control_deflections.update(delta)

    def update(self, deltas: ControlDeflections | None = None) -> AircraftStates:
        """
        ...
        Parameters
        ----------
        deltas : ControlDeflections, optional
        ...
        if not deltas is None:
            self.control_deflections = deltas
        u = self.forces_moments(self.state, self.control_deflections)
    ...
    def forces_moments(self, state: AircraftStates, deltas: ControlDeflections) -> np.ndarray:
        """
    ...
    def trim(
        ...
        def objective(y: np.ndarray) -> float:
            state = AircraftStates(y[0:12], use_quat=False)
            deltas = ControlDeflections(y[12:16])
        ...
        state_trim = AircraftStates(result.x[0:12], use_quat=False)
        deltas_trim = ControlDeflections(result.x[12:16])
