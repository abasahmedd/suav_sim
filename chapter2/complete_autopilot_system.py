...
from chapter1.airframe_parameters import AirframeParameters
...
from chapter2.longitudinal_lateral_controllers import LongitudinalLateralControl

AUTOPILOT_MODES = ["manual", "fbw", "cruise", "auto"]


class CompleteAutopilot:
    """
   ...
   7-  control_deflections : ControlDeflections
    ...
    def __init__(
        ...
        self.config = AutopilotDesignParameters()
        ...
        self.config.calculate_control_gains(
            params, AircraftStates(x_trim, use_quat=True), ControlDeflections(delta_trim)
        )

        self.flight_control = LongitudinalLateralControl(self.config)

        self.mode: str = None
        self.status = AutopilotStatusMonitoring(target_airspeed=state.airspeed)
        self.control_deflections = ControlDeflections(delta_trim, max_angle=np.deg2rad(45.0))

    def update(self, dt: float, state: AircraftStates) -> ControlDeflections:
        _dt = dt or self.dt
    ...
    def run_manual_mode(self, joystick: ControlDeflections) -> ControlDeflections:
        return ControlDeflections(joystick.delta, 0.25 * np.pi)

    def run_fbw_mode(
        self, dt: float, state: AircraftStates, joystick: ControlDeflections
    ) -> ControlDeflections:
        pass
    ...
    def run_auto_mode(self, dt: float, state: AircraftStates) -> ControlDeflections:
        pass
    ...
    def control_roll_pitch_airspeed(
        ...
        Then updates and returns `control_deflections` inputs.
        """
        ...
        self.control_deflections.delta_a = self.flight_control.roll_hold_with_aileron(
            self.status.target_roll, self.aircraft_state.roll, self.aircraft_state.p
        )
        self.control_deflections.delta_e = self.flight_control.pitch_hold_with_elevator(
            self.status.target_pitch, self.aircraft_state.pitch, self.aircraft_state.q
        )
        self.control_deflections.delta_r = self.flight_control.yaw_damper_with_rudder(
            self.aircraft_state.r, self.dt
        )
        self.control_deflections.delta_r += (
        ...
        self.control_deflections.delta_t = self.flight_control.airspeed_hold_with_throttle(
            self.status.target_airspeed, self.aircraft_state.airspeed, self.dt
        )

        return self.control_deflections
    ...
    def control_course_altitude_airspeed(
        ...
        Delegates to `control_roll_pitch_airspeed` to compute and updates `control_deflections` inputs.
        """
        # Update targets if new values are provided
        self.status.update_control_targets(
