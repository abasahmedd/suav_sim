...
from chapter2.longitudinal_lateral_controllers import FlightCommand
from chapter3.path.path_guidance import (
    LinePathParams,
    OrbitPathParams,
    PathGuidance,
)
from chapter3.path.navigation_algorithms import (
    DubinPathNavigator,
    FilletPathNavigator,
    LinePathNavigator,
    PathNavCommand,
...
from chapter3.path.waypoint_actions_manager import WaypointActionsManager
...
class MissionManagement:
    ...
    def __init__(
        self, dt: float, config: AutopilotDesignParameters, nav_type: str = "lines"
    ) -> None:
        """
        Initialize the MissionManagement with autopilot configuration and path type.
        ...
        self.path_follower = PathGuidance(config)
        self.route_manager = WaypointManagement(config)
    ...
    def _create_path_navigator(self, nav_type: str) -> NavigationAlgorithms:
