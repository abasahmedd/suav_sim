...
from chapter3.path.path_guidance import BasePathParams, LinePathParams, OrbitPathParams
...
class NavigationAlgorithms(ABC):
    """
    Abstract base class for path navigators that provide guidance for different path following strategies.
    """

    def __init__(
        self, config: AutopilotDesignParameters, route_manager: WaypointManagement = None
    ) -> None:
        """
        ...
        route_manager : WaypointManagement, optional
            Manager for handling waypoint navigation, by default None
        """
        self.config = config
        self.route_manager = route_manager or WaypointManagement(config)
...
class LinePathNavigator(NavigationAlgorithms):
    """
...
class FilletPathNavigator(NavigationAlgorithms):

    def __init__(self, config: AutopilotDesignParameters, route_manager: WaypointManagement) -> None:
    ...
    def navigate_path(self, pos_ned: np.ndarray, course: float = None) -> PathNavCommand:
        ...
        if self.route_manager.is_target_last():
            self.on_fillet = False
            self.nav_cmd.path_type = "line"
            self.nav_cmd.path_params = LinePathParams(wp0, q01)
            self.nav_cmd.is_new_path = False
            return self.nav_cmd

        q12 = (wp2 - wp1) / np.linalg.norm((wp2 - wp1))  # wp1 to wp2 direction
        dot_term = float(np.clip(-q01.T @ q12, -1.0, 1.0))
        fillet_angle = float(np.arccos(dot_term))

        if abs(np.sin(fillet_angle / 2.0)) < 1.0e-6 or np.linalg.norm(q01 - q12) < 1.0e-6:
            self.on_fillet = False
            self.nav_cmd.path_type = "line"
            self.nav_cmd.path_params = LinePathParams(wp0, q01)
            self.nav_cmd.is_new_path = False
        ...
        fillet_center = wp1 - fillet_radius / np.sin(fillet_angle / 2.0) * (
            q01 - q12
        ) / np.linalg.norm(q01 - q12)
        turn_direction = 1 if (q01[0] * q12[1] - q01[1] * q12[0]) >= 0.0 else -1
        line_to_orbit_transition = wp1 - (fillet_radius / np.tan(fillet_angle / 2.0)) * q01
        orbit_to_line_transition = wp1 + (fillet_radius / np.tan(fillet_angle / 2.0)) * q12
        ...
        if not self.on_fillet:
            ...
            if q01.dot(pos_ned - line_to_orbit_transition) > 0.0:
                self.on_fillet = True
                self.nav_cmd.path_type = "orbit"
                self.nav_cmd.path_params = OrbitPathParams(
                    fillet_center,
                    fillet_radius,
                    turn_direction,
                )
            ...
            # if not, keep following line path from wp0 to wp1
                ...
                self.on_fillet = False
                is_new_line = (
                    self.nav_cmd.path_type != "line" or self.nav_cmd.path_params is None
                )
                self.nav_cmd.path_type = "line"
                self.nav_cmd.path_params = LinePathParams(wp0, q01)
                self.nav_cmd.is_new_path = is_new_line

        # if following orbit path (fillet) around wp1
        else:
            # if the transition frontier from fillet to next line path is reached
            if q12.dot(pos_ned - orbit_to_line_transition) > 0.0:
                self.on_fillet = False
                self.nav_cmd.path_type = "line"
            ...
            # if not, keep following orbit path (fillet)
                ...
                self.on_fillet = True
                is_new_orbit = (
                    self.nav_cmd.path_type != "orbit" or self.nav_cmd.path_params is None
                )
                self.nav_cmd.path_type = "orbit"
                self.nav_cmd.path_params = OrbitPathParams(
                    fillet_center,
                ...
                self.nav_cmd.is_new_path = is_new_orbit

        return self.nav_cmd


class DubinPathNavigator(NavigationAlgorithms):
