from dataclasses import dataclass
from typing import Optional

import numpy as np

from chapter1.aircraft_states import AircraftStates
from .accelerometer_sensor import Accelerometer, AccelParams
from .airspeed_sensor import AirspeedSensor, AirspeedParams
from .barometer_sensor import Barometer, BaroParams
from .magnetometer_compass_sensor import MagnetometerCompass, CompassParams
from .global_positioning_system_sensor import GlobalPositioningSystem, GPSParams
from .gyroscope_sensor import Gyroscope, GyroParams
from .sensor import Sensor, SensorReading


@dataclass
class SensorReadings:
    accel: SensorReading
    gyro: SensorReading
    baro: SensorReading
    airspeed: SensorReading
    compass: SensorReading
    gps: SensorReading

    def as_array(self) -> np.ndarray:
        return np.hstack(
            [
                self.accel.data,
                self.gyro.data,
                self.baro.data,
                self.airspeed.data,
                self.compass.data,
                self.gps.data,
            ]
        )


class IntegratedSensorSystem:
    def __init__(
        self,
        state: AircraftStates,
        acc_params: Optional[AccelParams] = None,
        gyr_params: Optional[GyroParams] = None,
        baro_params: Optional[BaroParams] = None,
        airspeed_params: Optional[AirspeedParams] = None,
        compass_params: Optional[CompassParams] = None,
        gps_params: Optional[GPSParams] = None,
    ) -> None:
        self.state = state

        acc_params = acc_params or AccelParams()
        self.acc = Accelerometer(acc_params, state)

        gyr_params = gyr_params or GyroParams()
        self.gyr = Gyroscope(gyr_params, state)

        baro_params = baro_params or BaroParams()
        self.baro = Barometer(baro_params, state)

        airspeed_params = airspeed_params or AirspeedParams()
        self.airspeed = AirspeedSensor(airspeed_params, state)

        compass_params = compass_params or CompassParams()
        self.compass = MagnetometerCompass(compass_params, state)

        gps_params = gps_params or GPSParams()
        self.gps = GlobalPositioningSystem(gps_params, state)

        self.sensors: list[Sensor] = [
            self.acc,
            self.gyr,
            self.baro,
            self.airspeed,
            self.compass,
            self.gps,
        ]

    def initialize(self, t: float) -> None:
        for sensor in self.sensors:
            sensor.initialize(t)

    def update(self, t: float) -> None:
        for sensor in self.sensors:
            sensor.update(t)

    def read(self, t: float) -> SensorReadings:
        return SensorReadings(
            accel=self.acc.read(t),
            gyro=self.gyr.read(t),
            baro=self.baro.read(t),
            airspeed=self.airspeed.read(t),
            compass=self.compass.read(t),
            gps=self.gps.read(t),
        )
