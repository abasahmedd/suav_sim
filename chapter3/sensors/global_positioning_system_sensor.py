...
class GPSParams(SensorParams):
    """
    GlobalPositioningSystem configuration handler, children of `SensorParams`

    Attributes
    ----------
    sample_rate : float
        Sampling rate of the GlobalPositioningSystem sensor in Hz.
    reading_delay : float
        Delay in seconds before a GlobalPositioningSystem reading is available.
    ...
    time_constant : float
        ...
        GlobalPositioningSystem position error, in seconds.
    """
...
class GlobalPositioningSystem(Sensor):
    ...
    def __init__(
        self, params: GPSParams, state: AircraftStates, name: str = "gps"
    ) -> None:
        super().__init__(
            params,
            state,
