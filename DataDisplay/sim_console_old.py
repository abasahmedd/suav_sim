...
from chapter3.path.waypoints import Waypoint
from chapter3.path.waypoint_actions import OrbitTurns, OrbitTime, OrbitAlt, GoWaypoint, SetAirspeed
...
class SimConsoleOld:
    ...
    def __init__(self) -> None:
        """
        Initializes the SimConsoleLive with a Rich Console instance.
        """
    ...
    def print_control_deltas(self, deltas: ControlDeflections, style="simple") -> None:
        """
        ...
        deltas : ControlDeflections
            An instance of the ControlDeflections class containing control inputs.
        style : str, optional
            The style of output, either 'simple' or 'table' (default is 'simple').
    ...
    def print_aircraft_state(self, state: AircraftStates, style="simple") -> None:
        """
        ...
        state : AircraftStates
            An instance of the AircraftStates class representing the current state of the aircraft.
        style : str, optional
            The style of output, either 'simple' or 'table' (default is 'simple').
    ...
    def print_autopilot_status(self, status: AutopilotStatusMonitoring, style="simple") -> None:
        """
        Prints the current status of the autopilot.

        Parameters
        ----------
        status : AutopilotStatusMonitoring
            An instance of the AutopilotStatusMonitoring class representing the current autopilot status.
        style : str, optional
            The style of output, either 'simple' or 'table' (default is 'simple').
    ...
    def print_mission_status(self, mc: MissionManagement) -> None:
        self.console.print(
    ...
    def print_waypoints_table(self, rm: WaypointManagement) -> None:
        table = Table()
    ...
    def _print_control_deltas_simple(self, deltas: ControlDeflections) -> None:
        self.console.print(
    ...
    def _print_control_deltas_table(self, deltas: ControlDeflections) -> None:
        table = Table()
    ...
    def _print_aircraft_state_simple(self, state: AircraftStates) -> None:
        self.console.print(
    ...
    def _print_aircraft_state_table(self, state: AircraftStates) -> None:
        table = Table()
    ...
    def _print_autopilot_status_simple(self, status: AutopilotStatusMonitoring) -> None:
        self.console.print(
            "[bold magenta underline]CompleteAutopilot Status[/bold magenta underline]"
        )
        self.console.print(
        ...
        self.console.print(
            f"AirspeedSensor: {status.airspeed:.2f} m/s, Target AirspeedSensor: {status.target_airspeed:.2f} m/s, Error: {status.airspeed_error:.2f} m/s"
        )
        self.console.rule()

    def _print_autopilot_status_table(self, status: AutopilotStatusMonitoring) -> None:
        table = Table()

        # Define columns
        table.add_column("CompleteAutopilot Status", justify="left", style="cyan", no_wrap=True)
